...
public class TBEA09010000 {
	public static final String SERVICE_CODE = "TBEA09010000";
	...
	public ArrayList<KucunListItem> KucunList=new ArrayList<KucunListItem>();
	...
	public TBEA09010000(AsyncListener asynListener) {
	...
	class MeAsyncTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			pairs.add(new BasicNameValuePair("CommodityName", String.valueOf(CommodityName)));
			...
			try {
				...
				Totlecount=jobj.getString("Totlecount");
				KucunList=new ArrayList<KucunListItem>();
				if(JsonUtil.hasArray(jobj,"KucunList")){
					JSONArray jarray=jobj.getJSONArray("KucunList");
					for(int i=0;i<jarray.length();i++){
						KucunList.add(KucunListItem.initByJson(jarray.getJSONObject(i)));
					}
				}
			} catch (Exception e) {
				hasError = true;
