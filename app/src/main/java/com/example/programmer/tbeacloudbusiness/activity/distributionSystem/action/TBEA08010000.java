...
import com.example.programmer.tbeacloudbusiness.activity.distributionSystem.model.TotleInfo;
...
public class TBEA08010000 {
	public static final String SERVICE_CODE = "TBEA08010000";
	...
	public TotleInfo totleInfo;
	public ArrayList<OrderListItem> OrderList=new ArrayList<OrderListItem>();
	...
	public TBEA08010000(AsyncListener asynListener) {
	...
	class MeAsyncTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			try {
				...
				totleInfo=TotleInfo.initByJson(jobj.getJSONObject("TotleInfo"));
				if(JsonUtil.hasArray(jobj,"OrderList")){
					JSONArray jarray=jobj.getJSONArray("OrderList");
					for(int i=0;i<jarray.length();i++){
						OrderList.add(OrderListItem.initByJson(jarray.getJSONObject(i)));
					}
				}
			} catch (Exception e) {
				hasError = true;
